var app = getApp(), util = require("../../../utils/util.js"), date = new Date(), year = date.getFullYear(), month = date.getMonth() + 1, dayInMonth = date.getDate(), dayInWeek = date.getDay(), selected = [ year, month, dayInMonth ], week = [ {
    ...
    for (var a = getThisMonthDays(t, e), s = calEmptyGrid(t, e), n = [], o = 1; o <= a; o++) {
        var l = isToday(t, e, o), r = selected[0] == t && selected[1] == e && selected[2] == o, i = l ? "today" : "", c = r ? "selected" : "", d = {
            value: o,
            date: [ t, e, o ],
            class: "date-bg " + (isWeekend(s, o) ? "weekend" : "") + " " + i + " " + c + " " + (l && r ? "today-selected" : "")
        };
        n.push(d);
    }
...
Page({
    ...
    reLoad: function() {
        ...
        app.util.request({
            ...
            success: function(e) {
                console.log("我的签到", e), t.setData({
                    wdqd: e.data
                });
                for (var a = [], s = t.data.days, n = 0; n < e.data.length; n++) a.push(e.data[n].time);
                console.log(a, s), t.in_array(t.data.nowtime.toString(), a) ? (console.log("今日已签到"), 
                t.setData({
                    disabled: !0,
                    logintext: "今日已签到"
                ...
                for (var o = 0; o < s.length; o++) t.in_array(s[o].date.toString(), a) && (s[o].isqd = 1);
                app.util.request({
                    ...
                    success: function(e) {
                        ...
                        for (var a = e.data, n = 0; n < a.length; n++) {
                            a[n].day = a[n].day.split("-");
                            var o = new Date(a[n].day[0], a[n].day[1] - 1, a[n].day[2]), l = o.getFullYear(), r = o.getMonth() + 1, i = o.getDate();
                            a[n].day = l + "," + r + "," + i;
                        }
                        console.log(a), t.setData({
                            special: a
                        });
                        for (var c = 0; c < s.length; c++) for (var d = 0; d < a.length; d++) s[c].date.toString() == a[d].day && (s[c].tsrq = a[d]);
                        t.setData({
                            days: s
                        });
                    }
    ...
    qd: function() {
        var t = this, e = wx.getStorageSync("users").id, a = this.data.wdqd;
        ...
        if (0 == a.length) o = t.data.qdset[0].one; else var o = 0;
    ...
    bq: function() {
        var t = this, e = wx.getStorageSync("users").id, a = this.data.wdqd, s = Number(this.data.grjf);
        ...
        if (0 == a.length) l = t.data.qdset[0].one; else var l = 0;
        console.log(n, l), wx.showModal({
            title: "温馨提示",
    ...
    changeMonth: function(t) {
        ...
        s = "prev" == e ? s - 1 : s + 1, "prev" == e && s < 1 && (a -= 1, s = 12), "next" == e && s > 12 && (a += 1, 
        s = 1), this.setData({
            currYear: a,
            currMonth: s,
    ...
    selectDate: function(t) {
        ...
        var l = new Date(s[0], s[1], s[2]), r = new Date(n[0], n[1], n[2]), i = l.getTime(), c = r.getTime();
        console.log(i, c, a), i > c ? (console.log("以前"), 2 == a ? e.setData({
            bqdisabled: !1,
            bqtext: "点击补签"
        }) : e.setData({
            bqdisabled: !0,
