...
$wx = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and dn_state=2 and is_yue=2 and type=2 and pay_time !=''";
$wx = pdo_fetch($wx);//今天的微信店内销售额
$yue = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and dn_state=2 and is_yue=1 and type=2 and pay_time !=''";
$yue = pdo_fetch($yue);//今天的余额店内销售额
$jf = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and dn_state=2 and is_yue=3 and type=2 and pay_time !=''";
$jf = pdo_fetch($jf);//今天的积分店内销售额

$ztwx = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and dn_state=2 and is_yue=2 and type=2 and pay_time !=''";
$ztwx = pdo_fetch($ztwx);//昨天的微信店内销售额
$ztyue = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and dn_state=2 and is_yue=1 and type=2 and pay_time !=''";
$ztyue = pdo_fetch($ztyue);//昨天的余额店内销售额
$ztjf = "select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and dn_state=2 and is_yue=3 and type=2 and pay_time !=''";
...
$dn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."   and type=2 and dn_state=2 and uniacid=".$_W['uniacid'];
$dn2 = count(pdo_fetchall($dn2));//今天店内订单量
$wxdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."   and type=2 and dn_state=2 and is_yue=2 and uniacid=".$_W['uniacid'];
$wxdn2 = count(pdo_fetchall($wxdn2));//今天店内微信订单量
$yuedn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."   and type=2 and dn_state=2 and is_yue=1 and uniacid=".$_W['uniacid'];
$yuedn2 = count(pdo_fetchall($yuedn2));//今天店内余额订单量
$jfdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."   and type=2 and dn_state=2 and is_yue=3 and uniacid=".$_W['uniacid'];
$jfdn2 = count(pdo_fetchall($jfdn2));//今天店内积分订单量




$ztdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."   and type=2 and dn_state=2 and uniacid=".$_W['uniacid'];
$ztdn2 = count(pdo_fetchall($ztdn2));//昨天店内订单量
$ztwxdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."   and type=2 and dn_state=2 and is_yue=2 and uniacid=".$_W['uniacid'];
$ztwxdn2 = count(pdo_fetchall($ztwxdn2));//昨天店内微信订单量
$ztyuedn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."   and type=2 and dn_state=2 and is_yue=1 and uniacid=".$_W['uniacid'];
$ztyuedn2 = count(pdo_fetchall($ztyuedn2));//昨天店内余额订单量
$ztjfdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."   and type=2 and dn_state=2 and is_yue=3 and uniacid=".$_W['uniacid'];
...
// $dn="select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and dn_state not in (3,1) and type=2 and pay_time !='' and uniacid=".$_W['uniacid'];
// $dn = pdo_fetch($dn);//今天的店内销售额
// $ztdn="select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and dn_state not in (3,1) and type=2 and pay_time !='' and uniacid=".$_W['uniacid'];
// $ztdn = pdo_fetch($ztdn);//昨天的店内销售额

// $dn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and type=2 and uniacid=".$_W['uniacid'];
// $dn2 = count(pdo_fetchall($dn2));

// $ztdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and type=2 and uniacid=".$_W['uniacid'];
// $ztdn2 = count(pdo_fetchall($ztdn2));



if($_GPC['op']=='sc'){
  $result = pdo_delete('wpdc_order',array('dn_state'=>3));
    if($result){
...
if($_GPC['op']=='receivables'){
    $id=$_GPC['id'];
    $result = pdo_update('wpdc_order',array('dn_state'=>2), array('id'=>$id));
    if($result){
    ...
    // $data2['dn_state']=3;
    $result = pdo_update('wpdc_order',array('dn_state'=>3), array('id'=>$id));
...
if($_GPC['op']=='time'){
    $res=pdo_update('wpdc_order',array('pay_time'=>time()),array('id'=>$_GPC['id']));
     if($res){
        message('写入成功！', $this->createWebUrl('dnorder'), 'success');
    }else{
        message('写入失败！','','error');
    }
}
...
if(checksubmit('export_submit', true)) {
    $time=date("Y-m-d");
    $time="'%$time%'";
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("wpdc_order")." WHERE time LIKE ".$time."and type=2");
        ...
        for ($j = 1; $j <= $pagesize; $j++) {
            $sql = "select a.*,b.md_name,c.name as table_name from " . tablename("wpdc_order")."  a"  . " left join " . tablename("wpdc_store")." b on a.seller_id=b.id " . " left join " . tablename("wpdc_table") . " c on c.id=a.table_id  WHERE a.time LIKE ".$time." and a.type=2  limit " . ($j - 1) * 5000 . ",5000 ";
