...
// $dn="select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time."  and dn_state not in (3,1) and seller_id=".$storeid." and type=2 and pay_time !='' and uniacid=".$_W['uniacid'];
// $dn = pdo_fetch($dn);//今天的店内销售额
// $ztdn="select sum(money) as total from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."  and dn_state not in (3,1) and seller_id=".$storeid." and type=2 and pay_time !='' and uniacid=".$_W['uniacid'];
// $ztdn = pdo_fetch($ztdn);//昨天的店内销售额

// $dn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time."   and type=2  and seller_id=".$storeid."  and uniacid=".$_W['uniacid'];
// $dn2 = count(pdo_fetchall($dn2));

// $ztdn2 = "select * from " . tablename("wpdc_order")." WHERE time LIKE ".$time2."   and type=2 and seller_id=".$storeid." and uniacid=".$_W['uniacid'];
...
if($_GPC['op']=='time'){
    $res=pdo_update('wpdc_order',array('pay_time'=>time()),array('id'=>$_GPC['id']));
     if($res){
        message('写入成功！', $this->createWebUrl('indnorder'), 'success');
    }else{
        message('写入失败！','','error');
    }
}
