...
class zh_diancModuleSite extends Core {
    ...
    public function doMobileAddQd(){
    	...
    	if($res2){
    		...
    		pdo_update('wpdc_special',$data3,array('id'=>1));
    	}else{
    		$data3['day']=$_GPC['day'];
    		$data3['integral']=$_GPC['integral2'];
    		$data3['title']=$_GPC['title'];
...
if($yuser){
$sql =" select id,name from ".tablename('wpdc_user')." where uniacid={$_W['uniacid']}  and id not in ({$string}) and  name like '%{$_GPC['keywords']}%'";  
}else{
 $sql =" select id,name from ".tablename('wpdc_user')." where uniacid={$_W['uniacid']}   and  name like '%{$_GPC['keywords']}%'";     
}
